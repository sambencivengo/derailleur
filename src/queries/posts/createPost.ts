...
export async function createPost(postPayload: CreatePostPayload, userId: string, postId = uuid(), includeTags: boolean = true, attemptsLeft: number = 3): Promise<DerailleurResponse<PostWithTags>> {

  if (attemptsLeft < 1) {
    return (createErrorResponse([{ data: { postPayload, userId, postId, includeTags }, message: "Create Post recursive attempts exhausted" }]));
  }
  else {
    ...
    try {
      ...
      if (error.code == PrismaQueryErrorCodes.UNIQUE_CONSTRAINT && errorTarget[0] === 'name') {
        ...
        await createPost(postPayload, userId, postId, includeTags, attemptsLeft - 1);
      }
