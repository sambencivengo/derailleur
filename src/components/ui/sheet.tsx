...
const SheetContent = React.forwardRef<React.ElementRef<typeof SheetPrimitive.Content>, SheetContentProps>(({ side = 'right', setOpenState, className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    <SheetPrimitive.Content onInteractOutside={() => (setOpenState !== undefined ? setOpenState(false) : null)} ref={ref} className={cn(sheetVariants({ side }), className)} {...props}>
      {children}
      <SheetPrimitive.Close onClick={() => (setOpenState !== undefined ? setOpenState(false) : null)} className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-secondary">
        <X className="h-4 w-4" />
