  ...
  return (
    <div className="flex flex-col">
      ...
      {comments.map(({ _count, author, content, createdAt, id, postId, replies, updatedAt }, idx) => {
        return <Comment key={idx} author={author} commentId={id} content={content} createdAt={createdAt} postId={postId} replies={replies} updatedAt={updatedAt} repliesCount={_count.replies} user={user} level={0} />;
      })}
    </div>
  );
}
